/**
 * Plasma Bento — BentoGrid / BentoCard.
 *
 * Local fork of the ReactBits `MagicBento` pattern (that component is a
 * monolithic demo with baked-in content). Rebuilt around the real portfolio
 * content with three signature behaviours:
 *
 ...
 *
 * Every card carries its own `--glow-color` ("R, G, B") so orange, pink and
 * violet cards can coexist inside one grid.
 */
...
const Spotlight: React.FC<{
  ...
  useEffect(() => {
    ...
    const zeroCards = () => {
      section.querySelectorAll<HTMLElement>('.pb-bento-card').forEach((card) => {
...
}> = ({ children, className, glowColor = '139, 92, 246', spotlightRadius = SPOTLIGHT_RADIUS }) => {
  const sectionRef = useRef<HTMLDivElement>(null);
  const fxEnabled = useFxEnabled();

  return (
    <div ref={sectionRef} className={cn('pb-bento-section', className)}>
...
export const BentoCard: React.FC<BentoCardProps> = ({
  ...
  return (
    <div
      ref={ref}
      className={cn('pb-bento-card', className)}
