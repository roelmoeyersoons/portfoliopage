/**
 * Plasma Bento — Projects tab — "Plasma Slabs".
 *
 * Instead of the usual card grid: a vertical stack of full-width horizontal
 * slabs. Each slab is a BentoCard whose collapsed state is a single row —
 * gradient edge bar, mono index, title, category chip, mono tag previews and
 * a rotating chevron — and whose expanded state is a two-column dossier:
 * generated artwork (left) and tagline / highlights / tags / stats / repo
 * link (right). Accordion behaviour: at most one slab is open at a time.
 */
...
const Projects: React.FC<ProjectsTabProps> = ({ onNavigate }) => {
  ...
  return (
    <section className="mx-auto max-w-6xl px-5 pb-28 pt-28">
      ...
      <BentoGrid glowColor={GLOW.pink} className="flex flex-col gap-4">
        {projects.map((p, i) => {
            ...
            <motion.div key={p.id} custom={i} variants={slabV} initial="hidden" animate="show" className="min-w-0">
              <BentoCard glowColor={GLOWS[i % 3]} className="p-0">
                ...
                <button
                  ...
                  aria-expanded={open}
                  className={cn(
                    'relative flex w-full items-center gap-3 py-4 pl-6 pr-4 text-left outline-none transition-colors duration-300 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-pink-400/50 sm:gap-4 sm:py-5 sm:pl-7 sm:pr-5',
                ...
                >
                  ...
                  <span
                    className={cn(
                      ...
                      open
                        ? 'bg-gradient-to-b from-orange-300 to-pink-300 bg-clip-text text-transparent'
                        : 'text-white/30'
                    )}
                  >
                    {String(i + 1).padStart(2, '0')}
                  </span>
                  <span className="min-w-0 flex-1 truncate font-display text-[15px] font-bold tracking-tight text-white sm:text-lg">
                    {p.title}
                  </span>
                  <Chip className="shrink-0 !px-2 !text-[10px]">{p.category}</Chip>
                  <span className="hidden shrink-0 items-center gap-2 font-mono text-[10px] text-white/35 sm:flex">
                  ...
                  <ChevronDown
                    size={16}
                    className={cn('shrink-0 text-white/45 transition-transform duration-300', open && 'rotate-180')}
                  />
                </button>
                    ...
                    >
                      <div className="grid border-t border-white/[0.07] sm:grid-cols-2">
                        ...
                        <div className="flex min-w-0 flex-col p-5">
                          <p className="text-[13.5px] font-medium leading-relaxed text-white/85">{p.tagline}</p>
                          <ul className="mt-3 space-y-2">
                            {p.highlights.slice(0, 3).map((h, j) => (
                              <li key={j} className="flex gap-2.5 text-[13px] leading-relaxed text-white/55">
                          ...
                          <div className="mt-4 flex flex-wrap gap-1.5">
                            {p.tags.map((t) => (
                              <Chip key={t} className="!bg-white/[0.04] font-mono !text-[10px] text-white/50">
                                {t}
                              </Chip>
                            ))}
                          </div>
                          ...
                          {p.skillIds.length > 0 && (
                            <div className="mt-4 flex flex-wrap items-center gap-2">
                              <span className="inline-flex items-center gap-1 font-mono text-[10px] uppercase tracking-[0.18em] text-white/40">
                                <Zap size={11} className="text-pink-300/80" /> skills
                              </span>
                              {p.skillIds
                                ...
                                .map((skill) => (
                                  <button
                                    ...
                                    className="rounded-full border border-pink-400/25 bg-pink-500/10 px-3 py-1 text-[11px] font-medium text-pink-200 transition-colors hover:border-pink-400/50 hover:bg-pink-500/20"
                                  >
                                    {skill.title}
                                  </button>
                                ))}
                          ...
                          <div className="mt-auto flex flex-wrap items-end justify-between gap-x-5 gap-y-3 pt-4">
                            ...
                            {p.githubUrl && (
                              <a
                                ...
                                className="inline-flex items-center gap-1.5 text-xs font-semibold text-orange-300/90 transition hover:text-orange-200"
                              >
      ...
      <p className="mt-7 text-center font-mono text-[10px] tracking-[0.18em] text-white/30">
