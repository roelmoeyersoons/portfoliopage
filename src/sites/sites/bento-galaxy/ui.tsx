/**
 * Bento Galaxy — shared local primitives
 *
 * The bento structure's primitives, re-skinned to the deep-space observatory
 * palette: white/[0.04] glass panels on #05060d, white/[0.08] borders,
 * indigo #818cf8 / violet #a78bfa / cyan #67e8f9 accents, Space Grotesk
 * (font-display) headings, mono telemetry details.
 */
...
/** Glow colours as "R, G, B" triplets (drives every BentoCard glow) */
export const GLOW = {
  indigo: '129, 140, 248',
  violet: '167, 139, 250',
  cyan: '103, 232, 249',
...
/** Gradient text utility (light ends of the palette for the dark stage) */
export const TEXT_GRADIENT = 'bg-gradient-to-r from-indigo-300 via-violet-300 to-cyan-300 bg-clip-text text-transparent';
...
export const Metric: React.FC<{ label: string; value: string; delay?: number }> = ({ label, value, delay = 0 }) => {
  ...
  return (
    ...
    >
      <div className={cn('font-display text-xl font-semibold', TEXT_GRADIENT)}>
        {hasNumber && match ? (
          <>
            {match[1]}
